    ...
    fetch("/api/whatsapp-logs", {
      ...
      body: JSON.stringify({
        ...
        details: details.map((detail) => ({
          ...
          priceAtThatTime: detail.priceAtThatTime,
        })),
      }),
    }).catch(console.error);
        ...
        {!mounted || loading || (fetching && products.length === 0) ? (
          ...
          <div className="py-16 px-6 flex flex-col items-center justify-center text-center bg-white border border-border rounded-2xl max-w-2xl mx-auto">
            <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mb-6 text-text-muted border border-border shadow-2xs">
              <Heart className="size-7 opacity-45" />
            </div>
            ...
            <button
              ...
              className="px-8 py-3.5 bg-primary-500 text-white rounded-xl font-bold hover:bg-primary-600 transition-all cursor-pointer shadow-md"
            >
              Masuk dengan Google
            </button>
          </div>
          ...
          <>
            ...
            <div className="sticky top-20 z-40 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-4 bg-white/95 backdrop-blur-md rounded-2xl border border-border shadow-xs">
